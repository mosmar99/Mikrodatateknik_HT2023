...
void My_Delay(uint32_t mysec)
{
    ...
    // 1. Stoppa timern.
    TIM2->CR1 = TIM2->CR1 & ~TIM_CR1_CEN; // Återställ CEN (Counter Enable) till 0

    // 2. Nollställ räknaren.
    TIM2->CNT = 0; // Återställ räknaren till 0

    // 3. Starta timern.
    TIM2->CR1 = TIM2->CR1 | TIM_CR1_CEN; // Aktivera räknaren igen
    ...
    TIM2->CR1 = TIM2->CR1 & ~TIM_CR1_CEN; // Återställ CEN till 0 för att stoppa räknaren
}
